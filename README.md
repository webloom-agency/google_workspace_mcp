...
**3b. Slides fetches cover / free-floating images as an anonymous HTTP client.** Your app may return 200 to a logged-in browser, but Google's image servers send **no cookies and no `Authorization` header**. URLs that require a session, signed token, or `Referer` gate (common on `/api/file/…` asset hosts) produce a **broken-image triangle** in the editor even though `replaceImage` returned success. Fix on the **client / asset pipeline**: expose a **public** HTTPS PNG/JPEG, or upload to Drive and use a link-shared file URL.
...
**5b. Chart linking defaults to a static snapshot (`NOT_LINKED_IMAGE`).** This is deliberate: `LINKED` charts require **every person who opens the presentation** to have **read access** to the auxiliary data spreadsheet; otherwise Slides shows a broken-chart placeholder (black warning triangle) even though the Sheet tab looks fine when opened directly. The tool defaults to `NOT_LINKED_IMAGE` so client-facing decks render reliably. Opt back into live refresh with `deck.chart_linking_mode: "LINKED"` or per-chart `"linking_mode": "LINKED"` only when sharing rules guarantee spreadsheet access for all viewers.
